 ...
 * They have to be used, because using iostreams it is impossible to effectively
 * implement some operations. For example, using istream, you can not quickly
 * read string values from a tab-separated file, so that after reading, the
 * position remains immediately after the read value. (The only option is to
 * call the std::istream::get() function on each byte, but this slows down due
 * to several virtual calls.)
 *
 * Read/WriteBuffers provide direct access to the internal buffer, so the
 * necessary operations are implemented more efficiently. Only one virtual
 * function nextImpl() is used, which is rarely called:
 * - in the case of ReadBuffer - fill in the buffer with new data from the
 * source;
 ...
 * In the second case, you can effectively read from an already existing piece
 * of memory / std::string without copying it.
 */
class BufferBase {
   public:
     ...
     * offset - the starting point of the cursor. ReadBuffer must set it to the
     * end of the range, and WriteBuffer - to the beginning.
     */
    ...
    inline size_t available() const {
        return size_t(working_buffer.end() - pos);
    }
    ...
    /** How many bytes have been read/written, counting those that are still in
     * the buffer. */
   ...
   protected:
    void resetWorkingBuffer() {
        /// Move position to the end of buffer to trigger call of 'next' on next
        /// reading. Discard all data in current working buffer to prevent wrong
        /// assumptions on content of buffer, e.g. for optimizations of seeks in
        /// seekable buffers.
    ...
    /** How many bytes have been read/written, not counting those that are now
     * in the buffer. (counting those that were already used and "removed" from
     * the buffer)
     */
    size_t bytes = 0;

    /** A piece of memory that you can use.
     * For example, if internal_buffer is 1MB, and from a file for reading it
     * was loaded into the buffer only 10 bytes, then working_buffer will be 10
     * bytes in size (working_buffer.end() will point to the position
     * immediately after the 10 bytes that can be read).
     */
