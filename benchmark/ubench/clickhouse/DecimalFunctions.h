...
/// Traits used for determining final Type/Precision/Scale for certain math
...
struct DataTypeDecimalTrait {
    ...
    T scaleFactorFor(const DataTypeDecimalTrait<U> &x, bool) const {
        ...
        return DecimalUtils::scaleMultiplier<typename T::NativeType>(
            scale_delta);
    }
};

/// Calculates result = x * multiplier + delta.
/// If the multiplication or the addition overflows, returns false or throws
/// DECIMAL_OVERFLOW.
template <typename T, bool throw_on_error>
inline bool multiplyAdd(const T &x, const T &multiplier, const T &delta,
                        T &result) {
    T multiplied = 0;
    if (common::mulOverflow(x, multiplier, multiplied)) {
        if constexpr (throw_on_error) throw "Decimal math overflow";
        return false;
    }
 ...
 * multiplier. where scale_multiplier = scaleMultiplier<T>(scale) this is to
 * reduce number of calls to scaleMultiplier when scale is known.
 *
 * Sign of `whole` controls sign of result: negative whole => negative result,
 * positive whole => positive result. Sign of `fractional` is expected to be
 * positive, otherwise result is undefined. If `scale` is to big (scale >
 * max_precision<DecimalType::NativeType>), result is undefined.
 */
...
inline bool decimalFromComponentsWithMultiplierImpl(
    ...
    if (!multiplyAdd<T, throw_on_error>(
            whole, scale_multiplier,
            fractional_sign * (fractional % scale_multiplier), value))
        return false;
...
inline DecimalType decimalFromComponentsWithMultiplier(
    ...
    DecimalType result;
    decimalFromComponentsWithMultiplierImpl<DecimalType, true>(
        whole, fractional, scale_multiplier, result);
    return result;
}
...
inline bool tryGetDecimalFromComponentsWithMultiplier(
    ...
    const typename DecimalType::NativeType &fractional,
    typename DecimalType::NativeType scale_multiplier, DecimalType &result) {
    return decimalFromComponentsWithMultiplierImpl<DecimalType, false>(
        whole, fractional, scale_multiplier, result);
}
...
inline DecimalType decimalFromComponentsWithMultiplier(
    ...
    typename DecimalType::NativeType scale_multiplier) {
    return decimalFromComponentsWithMultiplier<DecimalType>(
        components.whole, components.fractional, scale_multiplier);
}
...
inline bool tryGetDecimalFromComponentsWithMultiplier(
    const DecimalComponents<DecimalType> &components,
    typename DecimalType::NativeType scale_multiplier, DecimalType &result) {
    return tryGetDecimalFromComponentsWithMultiplier<DecimalType>(
        components.whole, components.fractional, scale_multiplier, result);
}
...
inline DecimalType decimalFromComponents(
    const typename DecimalType::NativeType &whole,
    const typename DecimalType::NativeType &fractional, UInt32 scale) {
    using T = typename DecimalType::NativeType;

    return decimalFromComponentsWithMultiplier<DecimalType>(
        whole, fractional, scaleMultiplier<T>(scale));
}
...
inline bool tryGetDecimalFromComponents(
    ...
    return tryGetDecimalFromComponentsWithMultiplier<DecimalType>(
        whole, fractional, scaleMultiplier<T>(scale), result);
}
...
template <typename DecimalType>
inline DecimalType decimalFromComponents(
    const DecimalComponents<DecimalType> &components, UInt32 scale) {
    return decimalFromComponents<DecimalType>(components.whole,
                                              components.fractional, scale);
}
...
inline bool tryGetDecimalFromComponents(
    const DecimalComponents<DecimalType> &components, UInt32 scale,
    DecimalType &result) {
    return tryGetDecimalFromComponents<DecimalType>(
        components.whole, components.fractional, scale, result);
}

/** Split decimal into whole and fractional parts with given scale_multiplier.
 * This is an optimization to reduce number of calls to scaleMultiplier on known
 * scale.
 */
template <typename DecimalType>
...
/// Split decimal into components: whole and fractional part, @see
/// `DecimalComponents` for details.
template <typename DecimalType>
inline DecimalComponents<DecimalType> split(const DecimalType &decimal,
                                            UInt32 scale) {
    if (scale == 0) {
        return {decimal.value, 0};
    }
    return splitWithScaleMultiplier(
        decimal, scaleMultiplier<typename DecimalType::NativeType>(scale));
}
 ...
 */
template <typename DecimalType>
inline typename DecimalType::NativeType getWholePart(const DecimalType &decimal,
                                                     UInt32 scale) {
    if (scale == 0) return decimal.value;

    return decimal.value /
           scaleMultiplier<typename DecimalType::NativeType>(scale);
}
...
inline typename DecimalType::NativeType getFractionalPartWithScaleMultiplier(
    ...
    /// There's UB with min integer value here. But it does not matter for
    /// Decimals cause they use not full integer ranges. Anycase we make modulo
    /// before compare to make scale_multiplier > 1 unaffected.
 ...
 */
template <typename DecimalType>
inline typename DecimalType::NativeType getFractionalPart(
    const DecimalType &decimal, UInt32 scale) {
    if (scale == 0) return 0;

    return getFractionalPartWithScaleMultiplier(
        decimal, scaleMultiplier<typename DecimalType::NativeType>(scale));
}
...
ReturnType convertToImpl(const DecimalType &decimal, UInt32 scale, To &result) {
    ...
    if constexpr (std::is_floating_point_v<To>) {
        result = static_cast<To>(decimal.value) /
                 static_cast<To>(scaleMultiplier<DecimalNativeType>(scale));
    } else if constexpr (is_integer<To> &&
                         (sizeof(To) >= sizeof(DecimalNativeType))) {
        ...
        using CastTo = std::conditional_t<(is_big_int_v<DecimalNativeType> &&
                                           std::is_same_v<To, UInt8>),
                                          uint8_t, To>;
...
template <bool is_multiply, bool is_division, typename T, typename U,
          template <typename> typename DecimalType>
...
template <bool, bool, typename T, typename U,
          template <typename> typename DecimalType>
inline DataTypeDecimalTrait<T> binaryOpResult(const DecimalType<T> &tx,
                                              const DataTypeNumber<U> &) {
    return DataTypeDecimalTrait<T>(DecimalUtils::max_precision<T>,
                                   tx.getScale());
}

template <bool, bool, typename T, typename U,
          template <typename> typename DecimalType>
inline DataTypeDecimalTrait<U> binaryOpResult(const DataTypeNumber<T> &,
                                              const DecimalType<U> &ty) {
    return DataTypeDecimalTrait<U>(DecimalUtils::max_precision<U>,
                                   ty.getScale());
}
